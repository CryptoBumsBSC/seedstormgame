...
-   **Game Mechanics**:
    ...
    -   **Score Milestone Announcements**: At 100/250/500/1000/2000/3500/5000/7500/10000 the screen flashes a big pulsing label (e.g. `SHARP! 250`, `GRAND! 1000`, `10K LEGEND!`), a power-up chime plays, and screen-shake fires. Each milestone triggers once per run.
    -   **Combo Title Pop-ups**: Chain kills (≥3, 5, 7, 10, 15) trigger flashy centered labels (`TRIPLE!`, `RAMPAGE!`, `UNSTOPPABLE!`, `GODLIKE!`, `LEGENDARY!`). Reset on combo break or life lost.
    -   **Risk-Reward Zone**: Pulsing orange band across the top 80px of the canvas. Kills with enemy center inside the band score 2x base points; stacks with combo multiplier and daily modifier. Always-on, always free.
    -   **Daily Challenge Modifier** *(buff-only, by day of week)*: Sunday +50% power-up drops, Monday start with 4 lives, Tuesday 2x base points, Wednesday wingman from start, Thursday permanent rapid fire, Friday weapon unlocks at 70% of normal score thresholds, Saturday start with 6-sec shield. Shown as a glowing HUD badge above the game header and a 4-sec intro banner inside the canvas at run start.
